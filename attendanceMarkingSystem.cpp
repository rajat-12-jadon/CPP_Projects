...
void markAttendance()
{
    ...
    {
        cout << "Mark attendance for " << students[i].name << "(P/A): ";
        char status;
        cin >> status;
        cin.ignore();
        ...
        {
            cout << "Max Attendance records reached for " << students[i].name << ".\n";
            continue;
        }
        students[i].records[students[i].dateCount].date = date;
        students[i].records[students[i].dateCount].isPresent = (status == 'P' || status == 'p');
...
void displayRecords()
{
    ...
    for (int i = 0; i < studentCount; i++)
    {
        ...
        {
            cout << " Date: " << students[i].records[j].date << ", Status: " << (students[i].records[j].isPresent ? "Present" : "Absent") << "\n";
        }
    }
}
...
void viewOwnAttendance()
{
    ...
    {
        cout << " Date: " << students[idx].records[i].date << ", Status: " << (students[idx].records[i].isPresent ? "Present" : "Absent") << "\n";
    }
}
